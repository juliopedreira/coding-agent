...
[x] Add `lincona/openai_client/transport.py` defining `ResponsesTransport`, `HttpResponsesTransport` (httpx AsyncClient, headers, timeouts, single-attempt), and `MockResponsesTransport`.
[x] Add `lincona/openai_client/parsing.py` to parse streamed chunks (SSE-style `data:` lines) into `ResponseEvent` variants with tool-call lifecycle handling and size guards.
[x] Implement `OpenAIResponsesClient` in `lincona/openai_client/client.py` to assemble payloads, invoke transport, yield structured events, and map HTTP errors to domain errors.
[x] Wire configuration defaults (model, reasoning_effort, api_key) into client constructor; allow base_url/timeout overrides for tests/E2E.
[x] Add unit tests covering payload assembly, streaming text/tool calls, error mapping, cancellation/back-pressure, and mock transport behavior.
[x] Document usage (README or dev notes) and tick Epic 3 in `docs/mvp_00/01_PLAN.md` after completion.
...
## Pending follow-ups
- [x] Align streaming error handling with design (emit `ErrorEvent` instead of only raising exceptions, or update design to match behavior).
- [x] Add optional headers (`User-Agent`, `OpenAI-Beta`) and parse `Retry-After` when status is 429.
- [x] Provide bounded-queue helper for back-pressure (`consume_stream(queue_max=...)`) as documented.
- [x] Expose base_url override ergonomically via client factory/constructor (or document transport-only override).
- [x] Add optional logging hook to emit request id/status/latency per design notes.
