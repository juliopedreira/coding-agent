...
- Defaults (hardcoded): `model=gpt-5.1-codex-mini`, `reasoning_effort=medium`, `fs_mode=restricted`, `approval_policy=on-request`, `log_level=warning`, `allowed_models=["gpt-5","gpt-5-codex","gpt-5-mini","gpt-5-nano","gpt-5-pro","gpt-5.1","gpt-5.1-codex","gpt-5.1-codex-mini"]`.
...
- Config file layout:
  - `[auth]`: `api_key`
  - `[model]`: `id`, `reasoning_effort`, `allowed` (list of model ids for `/model:list` and `/model:set`)
...
- [x] Add config serialization back to TOML for future `lincona config` commands (optional).
- [x] Implement session ID generator (`YYYYMMDDHHMM-uuid4`).
- [x] Build JSONL event writer/reader with schema enforcement and append-only semantics.
- [x] Implement session list/resume/delete helpers; wire to CLI/TUI hooks.
- [x] Implement plaintext logger setup per session with size-based truncation safeguard.
- [x] Add graceful shutdown handler to flush JSONL/logs and close PTYs.
- [x] Write unit tests covering precedence resolution, permission enforcement, session ID format, JSONL schema, list/resume/delete, and log truncation.
